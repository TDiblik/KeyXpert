<script lang="ts">
  ...
  import type {ProfileDetailsInfo, ServiceConfig, UIConfig} from "./models";
  ...
  onMount(async () => {
    ...
    handle_tauri_result<UIConfig>(await invoke("get_ui_config", {}), (config) => {
      if (config.window_height == 0 && config.window_height == 0) {
        return; // config not yet edited, use defautl settings.
      }
      appWindow.setSize(new PhysicalSize(config.window_width, config.window_height));
      appWindow.setPosition(new PhysicalPosition(config.window_position_x, config.window_position_y));
    });
  ...
  async function check_for_updates() {
    ...
    if (!req.ok) {
      ...
      let err_text = "";
      try {
        err_text = JSON.stringify(await req.json());
      } catch {}
    ...
    modal_info.set({
      ...
      yes_callback: async () => {
        modal_info.set({
          ...
          description:
            "You can take a moment to make yourself a cup of coffee, as the update might take some time... (jk, it's blazingly fast, you probably didn't even manage to finish reading this)",
        } as ModalProps);

        const could_update = await invoke("download_and_install_update", {
          urlPath: download_url,
          expectedInstallerName: expected_installer_name,
        });
        if (!could_update) {
          modal_info.set({
            type: "error",
            ...
            description:
              "An error occurred during the installation of the new version. Please consider updating manually or attempting again later. Alternatively, you may find the new installer in your downloads folder, making it convenient for manual installation.",
          } as ModalProps);
        }
      },
    } as ModalProps);
    ...
    <div class="header-row">
      ...
      <div class="is-mapper-active-wrapper">
        <input
          type="checkbox"
          id="is-mapper-active"
          name="is-mapper-active"
          bind:checked={is_mapper_active}
          on:change={change_mapper_state}
        />
        <label for="is-mapper-active" id="mapper-state-label" class={`${!is_mapper_active ? "not-active" : ""}`}
          >Mapper is {is_mapper_active ? "active" : "inactive"}</label
        >
      </div>
    ...
    {#key selected_profile_id}
      {#if selected_profile_id?.length > 0}
        ...
        {#if selected_profile != null}
          <ProfileDetails active_profile={service_config.active_profile} {selected_profile} on_save={save_profile} />
        {/if}
      {/if}
    {/key}
